...
for(i in 1:nrow(M_val)){
	x <- M_val[i,]
	mod <- lm(x ~ relevel(ph2$tenown3c, ref = "3") + ph2$AgeMom + relevel(ph2$Eth9, ref = "1") + relevel(ph2$eduma, ref = "4") + relevel(ph2$smkpreg, ref = "2") + relevel(ph2$alpreg, ref = "0") + ph2$BMImom + ph2$Bcell + ph2$CD4T + ph2$CD8T + ph2$Eos + ph2$Mono + ph2$Neu + ph2$NK)
...
# write.table(resq, "Model10_New_Out/V4/model10c_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50 <- big_data %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50[1:50,], "Model10_New_Out/V4/model10c_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "results|big_data|qval|resq|top50"), file = "Model10_New_Out/V4/Model10c_unstrat_v4.RData")
...
for(i in 1:nrow(M_val)){
	x <- M_val[i,]
	mod <- lm(x ~ relevel(ph2$tenown3c, ref = "3") + ph2$AgeMom + relevel(ph2$Eth9, ref = "1") + relevel(ph2$eduma, ref = "4") + relevel(ph2$smkpreg, ref = "2") + relevel(ph2$alpreg, ref = "0") + ph2$BMImom + ph2$pregwks + ph2$Bcell + ph2$CD4T + ph2$CD8T + ph2$Eos + ph2$Mono + ph2$Neu + ph2$NK)
	results[[i]] <- summary(mod)$coef[2:3,]
	if(i %in% c(1, nrow(M_val))) {
		print(Sys.time())
		print("Model 10d")
...
qval <- big_data %>% select(., pvalue) %>% qvalue() %>% `[[`("qvalues") %>% rename(qval = pvalue) %>% rownames_to_column('dnam_id')
resq <- big_data %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% subset(., qval <= 0.05)
# write.table(resq, "Model10_New_Out/V4/model10d_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50 <- big_data %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50[1:50,], "Model10_New_Out/V4/model10d_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "results|big_data|qval|resq|top50"), file = "Model10_New_Out/V4/Model10d_unstrat_v4.RData")
