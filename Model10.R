...
## Model 10 - With DNA Methylation data
...
for(i in 1:nrow(M_val)){ #
	x <- M_val[i,]
	mod <- lm(x ~ relevel(ph2$tenown3c, ref = "3") + ph2$AgeMom + relevel(ph2$Eth9, ref = "1") + relevel(ph2$eduma, ref = "4") + ph2$Bcell + ph2$CD4T + ph2$CD8T + ph2$Eos + ph2$Mono + ph2$Neu + ph2$NK)
...
# write.table(resq, "Model10_New_Out/V4/model10_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50 <- big_data %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50[1:50,], "Model10_New_Out/V4/model10_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "results|big_data|qval|resq"), file = "Model10_New_Out/V4/Model10_unstrat_v4.RData")

#########################################################################
#########################################################################
#########################################################################

# Model 10a - Stratified by Offspring Sex, With DNA Methylation Data
...
for(i in 1:nrow(M_val)){
  ...
  mod <- lm(x ~ relevel(df$tenown3c, ref = "3") + df$AgeMom + relevel(df$Eth9, ref = "1") + relevel(df$eduma, ref = "4") + df$Bcell + df$CD4T + df$CD8T + df$Eos + df$Mono + df$Neu + df$NK)
  results_gen1[[i]] <- summary(mod)$coef[2:3,]
  if(i %in% c(1, nrow(M_val))) {
    print(Sys.time())
    print("Model 10a - Gender == 1")
  }
}
...
# write.table(resq_gen1, "Model10_New_Out/V4/model10a_gen1_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_gen1 <- big_data_gen1 %>% rownames_to_column('dnam_id') %>% left_join(., qval_gen1, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_gen1[1:50,], "Model10_New_Out/V4/model10_gen1_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "gen1"), file = "Model10_New_Out/V4/Model10_gen1_v4.RData")
...
# write.table(resq_gen2, "Model10_New_Out/V4/model10a_gen2_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_gen2 <- big_data_gen2 %>% rownames_to_column('dnam_id') %>% left_join(., qval_gen2, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_gen2[1:50,], "Model10_New_Out/V4/model10_gen2_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "gen2"), file = "Model10_New_Out/V4/Model10_gen2_v4.RData")

#########################################################################
#########################################################################
#########################################################################

## Model 10b - Stratified by Mother's Ethnicity, With DNA Methylation Data
...
for(i in 1:nrow(M_val)){
  ...
  mod <- lm(x ~ relevel(df$tenown3c, ref = "3") + df$AgeMom + relevel(df$eduma, ref = "4") + df$Bcell + df$CD4T + df$CD8T + df$Eos + df$Mono + df$Neu + df$NK)
  results_eth1[[i]] <- summary(mod)$coef[2:3,]
  if(i %in% c(1, nrow(M_val))) {
    print(Sys.time())
    print("Model 10b - Ethnicity == 1")
  }
}
...
# write.table(resq_eth1, "Model10_New_Out/V4/model10b_eth1_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_eth1 <- big_data_eth1 %>% rownames_to_column('dnam_id') %>% left_join(., qval_eth1, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_eth1[1:50,], "Model10_New_Out/V4/model10_eth1_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "eth1"), file = "Model10_New_Out/V4/Model10_eth1_v4.RData")
...
# write.table(resq_eth7, "Model10_New_Out/V4/model10b_eth7_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_eth7 <- big_data_eth7 %>% rownames_to_column('dnam_id') %>% left_join(., qval_eth7, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_eth7[1:50,], "Model10_New_Out/V4/model10_eth7_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "eth7"), file = "Model10_New_Out/V4/Model10_eth7_v4.RData")