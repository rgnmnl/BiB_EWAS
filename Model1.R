...
## Title: Model 1 EWAS
## Version: 4
## Note: Updated based on EWAS_analysis_planv4. Changed pheno file.
...
for(i in 1:nrow(M_val)){
	x <- M_val[i,]
	mod <- lm(x ~ relevel(ph2$eduma_di, ref = "2") + ph2$AgeMom + relevel(ph2$Eth9, ref = "1") + ph2$Bcell + ph2$CD4T + ph2$CD8T + ph2$Eos + ph2$Mono + ph2$Neu + ph2$NK)
...
write.table(resq, "Model3_New_Out/V4/model3_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50 <- big_data %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
write.table(top50[1:50,], "Model3_New_Out/V4/model3_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "results|big_data|qval|resq"), file = "Model3_New_Out/V4/Model3_unstrat_v4.RData")
...
for(i in 1:nrow(M_val)){
  ...
  mod <- lm(x ~ relevel(df$eduma_di, ref = "2") + df$AgeMom + relevel(df$Eth9, ref = "1") + df$Bcell + df$CD4T + df$CD8T + df$Eos + df$Mono + df$Neu + df$NK)
...
# write.table(resq_gen1, "Model3_New_Out/V4/model3a_gen1_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_gen1 <- big_data_gen1 %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_gen1[1:50,], "Model3_New_Out/V4/model3_gen1_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "gen1"), file = "Model3_New_Out/V4/Model3_gen1_v4.RData")
...
# write.table(resq_gen2, "Model3_New_Out/V4/model3a_gen2_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_gen2 <- big_data_gen2 %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_gen2[1:50,], "Model3_New_Out/V4/model3_gen2_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "gen2"), file = "Model3_New_Out/V4/Model3_gen2_v4.RData")
...
for(i in 1:nrow(M_val)){
  ...
  mod <- lm(x ~ relevel(df$eduma_di, ref = "2") + df$AgeMom + df$Bcell + df$CD4T + df$CD8T + df$Eos + df$Mono + df$Neu + df$NK)
...
# write.table(resq_eth1, "Model3_New_Out/V4/model3b_eth1_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_eth1 <- big_data_eth1 %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_eth1[1:50,], "Model3_New_Out/V4/model3_eth1_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "eth1"), file = "Model3_New_Out/V4/Model3_eth1_v4.RData")
...
# write.table(resq_eth7, "Model3_New_Out/V4/model3b_eth7_qfilt_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_eth7 <- big_data_eth7 %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_eth7[1:50,], "Model3_New_Out/V4/model3_eth7_top50_v4.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "eth7"), file = "Model3_New_Out/V4/Model3_eth7_v4.RData")