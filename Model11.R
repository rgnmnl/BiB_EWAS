...
## Model 11 - With DNA Methylation data
...
for(i in 1:nrow(M_val)){ #
	x <- M_val[i,]
	mod <- lm(x ~ relevel(ph2$quin073c, ref = "3") + ph2$AgeMom + relevel(ph2$Eth9, ref = "1") + relevel(ph2$eduma, ref = "4") + ph2$Bcell + ph2$CD4T + ph2$CD8T + ph2$Eos + ph2$Mono + ph2$Neu + ph2$NK)
...
# write.table(resq, "Model11_New_Out/V5/model11_qfilt_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50 <- big_data %>% rownames_to_column('dnam_id') %>% left_join(., qval, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50[1:50,], "Model11_New_Out/V5/model11_top50_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "results|big_data|qval|resq"), file = "Model11_New_Out/V5/Model11_unstrat_v5.RData")

#########################################################################
#########################################################################
#########################################################################

# Model 11a - Stratified by Offspring Sex, With DNA Methylation Data
...
for(i in 1:nrow(M_val)){
  ...
  mod <- lm(x ~ relevel(df$quin073c, ref = "3") + df$AgeMom + relevel(df$Eth9, ref = "1") + relevel(df$eduma, ref = "4") + df$Bcell + df$CD4T + df$CD8T + df$Eos + df$Mono + df$Neu + df$NK)
  results_gen1[[i]] <- summary(mod)$coef[2:3,]
  if(i %in% c(1, nrow(M_val))) {
    print(Sys.time())
    print("Model 11a - Gender == 1")
  }
}
...
# write.table(resq_gen1, "Model11_New_Out/V5/model11a_gen1_qfilt_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_gen1 <- big_data_gen1 %>% rownames_to_column('dnam_id') %>% left_join(., qval_gen1, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_gen1[1:50,], "Model11_New_Out/V5/model11_gen1_top50_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "gen1"), file = "Model11_New_Out/V5/Model11_gen1_v5.RData")
...
# write.table(resq_gen2, "Model11_New_Out/V5/model11a_gen2_qfilt_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_gen2 <- big_data_gen2 %>% rownames_to_column('dnam_id') %>% left_join(., qval_gen2, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_gen2[1:50,], "Model11_New_Out/V5/model11_gen2_top50_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "gen2"), file = "Model11_New_Out/V5/Model11_gen2_v5.RData")

#########################################################################
#########################################################################
#########################################################################

## Model 11b - Stratified by Mother's Ethnicity, With DNA Methylation Data
...
for(i in 1:nrow(M_val)){
  ...
  mod <- lm(x ~ relevel(df$quin073c, ref = "3") + df$AgeMom + relevel(df$eduma, ref = "4") + df$Bcell + df$CD4T + df$CD8T + df$Eos + df$Mono + df$Neu + df$NK)
  results_eth1[[i]] <- summary(mod)$coef[2:3,]
  if(i %in% c(1, nrow(M_val))) {
    print(Sys.time())
    print("Model 11b - Ethnicity == 1")
  }
}
...
# write.table(resq_eth1, "Model11_New_Out/V5/model11b_eth1_qfilt_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_eth1 <- big_data_eth1 %>% rownames_to_column('dnam_id') %>% left_join(., qval_eth1, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_eth1[1:50,], "Model11_New_Out/V5/model11_eth1_top50_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "eth1"), file = "Model11_New_Out/V5/Model11_eth1_v5.RData")
...
# write.table(resq_eth7, "Model11_New_Out/V5/model11b_eth7_qfilt_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

top50_eth7 <- big_data_eth7 %>% rownames_to_column('dnam_id') %>% left_join(., qval_eth7, by = "dnam_id") %>% arrange(pvalue)
# write.table(top50_eth7[1:50,], "Model11_New_Out/V5/model11_eth7_top50_v5.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "eth7"), file = "Model11_New_Out/V5/Model11_eth7_v5.RData")
