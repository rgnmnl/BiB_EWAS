...
## Title: Model 6 EWAS
## Version: 1 
## Note: Originally Model 5 - Version 1, but shifted to Model 6
...
#Model 5: Index of multiple deprivation 2007 -> DNAm (also adjusted for mother’s age and ethnicity as well as cell type composition)
...
for(i in 1:nrow(M_val)){
	x <- M_val[i,]
	mod <- lm(x ~ relevel(ph2$empstma, ref = "5") + ph2$AgeMom + relevel(ph2$Eth9, ref = "1") + relevel(ph2$eduma, ref = "4") + ph2$Bcell + ph2$CD4T + ph2$CD8T + ph2$Eos + ph2$Mono + ph2$Neu + ph2$NK)
...
# write.table(resq, "model5_pfilt_results.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
# write.table(top50[1:50,], "model5_top50.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "big_data|qval|resq"), file = "model5_unstrat.RData")
...
for(i in 1:nrow(M_val)){ #
  ...
  mod <- lm(x ~ relevel(df$empstma, ref = "5") + df$AgeMom + relevel(df$Eth9, ref = "1") + relevel(df$eduma, ref = "4") + df$Bcell + df$CD4T + df$CD8T + df$Eos + df$Mono + df$Neu + df$NK)
...
# write.table(resq_gen1, "model5a_gen1_pfilt.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
# write.table(top50_gen1[1:50,], "model5_gen1_top50.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "gen1"), file = "model5_gen1.RData")
...
# write.table(resq_gen2, "model5a_gen2_pfilt.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
# write.table(top50_gen2[1:50,], "model5_gen2_top50.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "gen2"), file = "model5_gen2.RData")

#########################################################################
#########################################################################
#########################################################################

## Model 5b - Stratified by Mother's Ethnicity, With DNA Methylation Data
...
for(i in 1:nrow(M_val)){
  ...
  mod <- lm(x ~ relevel(df$empstma, ref = "5") + df$AgeMom + relevel(df$eduma, ref = "4") + df$Bcell + df$CD4T + df$CD8T + df$Eos + df$Mono + df$Neu + df$NK)
...
# write.table(resq_eth1, "model5b_eth1_pfilt.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
# write.table(top50_eth1[1:50,], "model5_eth1_top50.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
...
# write.table(resq_eth7, "model5b_eth7_pfilt.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
# write.table(top50_eth7[1:50,], "model5_eth7_top50.txt", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Export model results to RData file
save(list = ls(pattern = "eth7"), file = "model5_eth7.RData")
